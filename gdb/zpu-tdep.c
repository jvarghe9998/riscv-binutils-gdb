...
struct zpu_frame_cache
{
  CORE_ADDR base;
  struct trad_frame_saved_reg *saved_regs;
};

static const char * const zpu_gdb_reg_names[ZPU_LAST_FP_REGNUM + 1] =
{
...
static const struct register_alias zpu_register_aliases[] =
{
  { "zero", 0 },
  { "ra", 1 },
  { "sp", 2 },
...
static struct cmd_list_element *setzpucmdlist = NULL;
static struct cmd_list_element *showzpucmdlist = NULL;

static void
show_zpu_command (char *args, int from_tty)
{
  help_list (showzpucmdlist, "show zpu ", all_commands, gdb_stdout);
}

static void
set_zpu_command (char *args, int from_tty)
{
  printf_unfiltered
    ("\"set zpu\" must be followed by an appropriate subcommand.\n");
  help_list (setzpucmdlist, "set zpu ", all_commands, gdb_stdout);
}
...
zpu_sw_breakpoint_from_kind (struct gdbarch *gdbarch, int kind, int *size)
{
...
static struct value *
value_of_zpu_user_reg (struct frame_info *frame, const void *baton)
{
	       ...
	       int prefer_alias)
{
  ...
  if (prefer_alias &&
      regnum >= ZPU_ZERO_REGNUM && regnum <= ZPU_LAST_REGNUM)
    {
      for (i = 0; i < ARRAY_SIZE (zpu_register_aliases); ++i)
	if (regnum == zpu_register_aliases[i].regnum)
	  return zpu_register_aliases[i].name;
    }

  if (regnum >= ZPU_ZERO_REGNUM && regnum <= ZPU_LAST_FP_REGNUM)
      return zpu_gdb_reg_names[regnum];

  if (regnum >= ZPU_FIRST_CSR_REGNUM && regnum <= ZPU_LAST_CSR_REGNUM)
    {
      sprintf(buf, "csr%d", regnum - ZPU_FIRST_CSR_REGNUM);
      return buf;
    }

  if (regnum == ZPU_PRIV_REGNUM)
    {
      return "priv";
    }
...
static const char *
zpu_register_name (struct gdbarch *gdbarch,
		     int regnum)
{
...
static void
zpu_extract_return_value (struct type *type,
...
static void
zpu_store_return_value (struct type *type,
			  struct regcache *regs,
			  const gdb_byte *src,
			  int regnum)
{
...
zpu_return_value (struct gdbarch  *gdbarch,
		    ...
		    const gdb_byte *writebuf)
{
  ...
  if (rv_size > 2 * zpu_isa_regsize (gdbarch))
    {
      if (readbuf || writebuf)
	regcache_cooked_read_unsigned (regcache, ZPU_A0_REGNUM, &tmp);
      if (readbuf)
  ...
  regnum = fp ? ZPU_FA0_REGNUM : ZPU_A0_REGNUM;

  if (readbuf)
    zpu_extract_return_value (type, regcache, readbuf, regnum);

  if (writebuf)
    zpu_store_return_value (type, regcache, writebuf, regnum);
...
zpu_pseudo_register_read (struct gdbarch *gdbarch,
			    struct regcache *regcache,
			    int regnum,
			    gdb_byte *buf)
{
...
static void
zpu_pseudo_register_write (struct gdbarch *gdbarch,
			     struct regcache *regcache,
			     int cookednum,
			     const gdb_byte *buf)
{
		     ...
		     int regnum)
{
  ...
  else if (regnum <= ZPU_LAST_FP_REGNUM)
    {
      switch (regsize)
	{
	case 4:
  ...
  else if (regnum == ZPU_PRIV_REGNUM)
    {
      return builtin_type (gdbarch)->builtin_int8;
    }
  else
    {
      if (regnum == ZPU_CSR_FFLAGS_REGNUM
	  || regnum == ZPU_CSR_FRM_REGNUM
	  || regnum == ZPU_CSR_FCSR_REGNUM)
...
static void
zpu_print_fp_register (struct ui_file *file, struct frame_info *frame,
			 int regnum)
{
				...
				int regnum)
{
  ...
  if (regnum >= ZPU_FIRST_FP_REGNUM && regnum <= ZPU_LAST_FP_REGNUM)
    zpu_print_fp_register (file, frame, regnum);
  else
    {
      /* Integer type.  */
      int offset, size;
	...
	offset = 0;
      ...
      print_scalar_formatted (raw_buffer + offset,
			      ...
			      size == 8 ? 'g' : 'w', file);
      fprintf_filtered (file, "\t");
      if (size == 4 && zpu_isa_regsize (gdbarch) == 8)
	fprintf_filtered (file, "\t");

      if (regnum == ZPU_CSR_MSTATUS_REGNUM)
	{
	  if (size == 4)
      ...
      else if (regnum == ZPU_CSR_MISA_REGNUM)
        {
          int base;
          if (size == 4) {
            d = unpack_long (builtin_type (gdbarch)->builtin_uint32, raw_buffer);
      ...
      else if (regnum == ZPU_CSR_FCSR_REGNUM
	       || regnum == ZPU_CSR_FFLAGS_REGNUM
	       || regnum == ZPU_CSR_FRM_REGNUM)
	{
	  d = unpack_long (builtin_type (gdbarch)->builtin_int32, raw_buffer);

	  if (regnum != ZPU_CSR_FRM_REGNUM)
	  ...
	  if (regnum != ZPU_CSR_FFLAGS_REGNUM)
		...
		"dynamic rounding mode",
	      ...
	      int frm = ((regnum == ZPU_CSR_FCSR_REGNUM) ? (d >> 5) : d) & 0x3;
...
static int
zpu_register_reggroup_p (struct gdbarch  *gdbarch,
			   int regnum,
			   struct reggroup *reggroup)
{
  ...
  if (reggroup == all_reggroup) {
    if (regnum < ZPU_FIRST_CSR_REGNUM || regnum == ZPU_PRIV_REGNUM)
      return 1;
    ...
    return (regnum >= ZPU_FIRST_FP_REGNUM && regnum <= ZPU_LAST_FP_REGNUM)
	    || (regnum == ZPU_CSR_FCSR_REGNUM
	        || regnum == ZPU_CSR_FFLAGS_REGNUM
	        || regnum == ZPU_CSR_FRM_REGNUM);
  else if (reggroup == general_reggroup)
    return regnum < ZPU_FIRST_FP_REGNUM;
  else if (reggroup == restore_reggroup || reggroup == save_reggroup)
    return regnum <= ZPU_LAST_FP_REGNUM;
...
static void
zpu_print_registers_info (struct gdbarch    *gdbarch,
			    struct ui_file    *file,
			    ...
			    int                all)
{
  ...
  if (regnum != -1)
    {
      ...
      gdb_assert (regnum <= ZPU_LAST_REGNUM);
      if (NULL == register_name (gdbarch, regnum, 1))
        error (_("Not a valid register for the current processor type"));
      zpu_print_register_formatted (file, frame, regnum);
      return;
    }
  ...
  for (regnum = 0; regnum <= ZPU_LAST_REGNUM; ++regnum)
    {
      /* Zero never changes, so might as well hide by default.  */
      if (regnum == ZPU_ZERO_REGNUM && !all)
        continue;
      if (zpu_register_reggroup_p(gdbarch, regnum, reggroup))
        zpu_print_register_formatted (file, frame, regnum);
    }
}

static ULONGEST
zpu_fetch_instruction (struct gdbarch *gdbarch, CORE_ADDR addr)
{
  ...
  instlen = zpu_insn_length (buf[0]);
  if (instlen > sizeof (buf))
    internal_error (__FILE__, __LINE__, _("%s: zpu_insn_length returned %i"),
		    __func__, instlen);
  else if (instlen > 2)
    {
...
static void
set_reg_offset (struct gdbarch *gdbarch, struct zpu_frame_cache *this_cache,
...
static void
reset_saved_regs (struct gdbarch *gdbarch, struct zpu_frame_cache *this_cache)
{
...
static int zpu_decode_register_index(unsigned long opcode, int offset)
{
    return (opcode >> offset) & 0x1F;
}

static CORE_ADDR
zpu_scan_prologue (struct gdbarch *gdbarch,
		     CORE_ADDR start_pc, CORE_ADDR limit_pc,
		     struct frame_info *this_frame,
		     struct zpu_frame_cache *this_cache)
{
  CORE_ADDR cur_pc;
  CORE_ADDR frame_addr = 0;
  CORE_ADDR sp;
  long frame_offset;
  int frame_reg = ZPU_SP_REGNUM;
  ...
  if (this_frame != NULL)
    sp = get_frame_register_signed (this_frame, ZPU_SP_REGNUM);
  else
    sp = 0;
 ...
 restart:
  ...
  for (cur_pc = start_pc; cur_pc < limit_pc; cur_pc += 4)
    {
      ...
      inst = zpu_fetch_instruction (gdbarch, cur_pc);

      /* Decode the instruction.  These offsets are defined in the RISC-V ISA
       * manual.  */
      reg = zpu_decode_register_index(inst, 7);
      rs1 = zpu_decode_register_index(inst, 15);
      rs2 = zpu_decode_register_index(inst, 20);
      imm12 = (inst >> 20) & 0xFFF;
	  ...
	  && reg == ZPU_SP_REGNUM && rs1 == ZPU_SP_REGNUM)
	{
      ...
      else if (is_sw_insn(inst) && rs1 == ZPU_SP_REGNUM)
	{
	  /* sw reg, offset(sp) */
	  set_reg_offset (gdbarch, this_cache, rs1, sp + offset12);
	}
      else if (is_sd_insn(inst) && rs1 == ZPU_SP_REGNUM)
	{
	  /* sd reg, offset(sp) */
	  set_reg_offset (gdbarch, this_cache, rs1, sp + offset12);
	}
      else if (is_addi_insn(inst) && reg == ZPU_FP_REGNUM
	       && rs1 == ZPU_SP_REGNUM)
	{
	  /* addi s0, sp, size */
      ...
      else if (this_frame && frame_reg == ZPU_SP_REGNUM)
	{
	  unsigned alloca_adjust;

	  frame_reg = ZPU_FP_REGNUM;
	  frame_addr = get_frame_register_signed (this_frame, ZPU_FP_REGNUM);
	  ...
	  if (alloca_adjust > 0)
	    {
	      ...
	      goto restart;
	       ...
	       && reg == ZPU_FP_REGNUM && rs1 == ZPU_SP_REGNUM
               && rs2 == ZPU_ZERO_REGNUM)
	{
	  /* add s0, sp, 0 */
	  /* addw s0, sp, 0 */
	  if (this_frame && frame_reg == ZPU_SP_REGNUM)
	    {
	      unsigned alloca_adjust;
	      frame_reg = ZPU_FP_REGNUM;
      ...
      else if (is_sw_insn(inst) && rs1 == ZPU_FP_REGNUM)
	{
	  /* sw reg, offset(s0) */
	  set_reg_offset (gdbarch, this_cache, rs1, frame_addr + offset12);
	}
      else if (reg == ZPU_GP_REGNUM
	       && (is_auipc_insn(inst)
                   || is_lui_insn(inst)
		   || (is_addi_insn(inst) && rs1 == ZPU_GP_REGNUM)
		   || (is_add_insn(inst) && (rs1 == ZPU_GP_REGNUM
					  || rs2 == ZPU_GP_REGNUM))))
	{
	  /* auipc gp, n */
	  /* addi gp, gp, n */
  ...
  if (this_cache != NULL)
	...
	+ frame_offset;
      this_cache->saved_regs[ZPU_PC_REGNUM] =
	this_cache->saved_regs[ZPU_RA_REGNUM];
    }
...
/* Implement the zpu_skip_prologue gdbarch method.  */

static CORE_ADDR
zpu_skip_prologue (struct gdbarch *gdbarch,
		     CORE_ADDR       pc)
{
  CORE_ADDR limit_pc;
  CORE_ADDR func_addr;
  ...
  return zpu_scan_prologue (gdbarch, pc, limit_pc, NULL, NULL);
}

static CORE_ADDR
zpu_push_dummy_code (struct gdbarch *gdbarch, CORE_ADDR sp, CORE_ADDR funaddr,
...
static CORE_ADDR
zpu_push_dummy_call (struct gdbarch *gdbarch,
		       struct value *function,
		       struct regcache *regcache,
		       CORE_ADDR bp_addr,
		       int nargs,
  ...
  for (i = 0; i < nargs && i < 8; ++i)
    {
      ...
      int regnum = (TYPE_CODE (value_type) == TYPE_CODE_FLT
		    ? ZPU_FA0_REGNUM : ZPU_A0_REGNUM);

      regcache_cooked_write_unsigned
	(regcache, regnum + i,
	 extract_unsigned_integer
	   (arg_bits, zpu_isa_regsize(gdbarch), byte_order));
    }

  /* Store struct value address.  */

  if (struct_return)
    regcache_cooked_write_unsigned (regcache, ZPU_A0_REGNUM, struct_addr);

  /* Set the dummy return value to bp_addr.
     A dummy breakpoint will be setup to execute the call.  */

  regcache_cooked_write_unsigned (regcache, ZPU_RA_REGNUM, bp_addr);

  /* Finally, update the stack pointer.  */

  regcache_cooked_write_unsigned (regcache, ZPU_SP_REGNUM, sp);
...
static CORE_ADDR
zpu_frame_align (struct gdbarch *gdbarch, CORE_ADDR addr)
{
...
{
  return frame_unwind_register_unsigned (next_frame, ZPU_PC_REGNUM);
}
...
{
  return frame_unwind_register_unsigned (next_frame, ZPU_SP_REGNUM);
}

/* Implement the dummy_id gdbarch method.  */

static struct frame_id
zpu_dummy_id (struct gdbarch *gdbarch, struct frame_info *this_frame)
{
  return frame_id_build (get_frame_register_signed (this_frame, ZPU_SP_REGNUM),
			 get_frame_pc (this_frame));
}

static struct trad_frame_cache *
zpu_frame_cache (struct frame_info *this_frame, void **this_cache)
{
  CORE_ADDR pc;
  ...
  stack_addr = get_frame_register_signed (this_frame, ZPU_SP_REGNUM);
...
static void
zpu_frame_this_id (struct frame_info *this_frame,
		     void              **prologue_cache,
		     struct frame_id   *this_id)
{
  struct trad_frame_cache *info = zpu_frame_cache (this_frame, prologue_cache);
  trad_frame_get_id (info, this_id);
}

static struct value *
zpu_frame_prev_register (struct frame_info *this_frame,
			   void              **prologue_cache,
			   int                regnum)
{
  struct trad_frame_cache *info = zpu_frame_cache (this_frame, prologue_cache);
...
static const struct frame_unwind zpu_frame_unwind =
{
  ...
  /*.this_id       =*/ zpu_frame_this_id,
  /*.prev_register =*/ zpu_frame_prev_register,
  /*.unwind_data   =*/ NULL,
		    ...
		    struct gdbarch_list *arches)
{
  ...
  for (arches = gdbarch_list_lookup_by_info (arches, &info);
    ...
    if (gdbarch_tdep (arches->gdbarch)->zpu_abi == abi)
  ...
  set_gdbarch_long_bit (gdbarch, zpu_isa_regsize (gdbarch) * 8);
  set_gdbarch_float_bit (gdbarch, 32);
  set_gdbarch_double_bit (gdbarch, 64);
  set_gdbarch_long_double_bit (gdbarch, 128);
  set_gdbarch_ptr_bit (gdbarch, zpu_isa_regsize (gdbarch) * 8);
  set_gdbarch_char_signed (gdbarch, 1);

  /* Information about the target architecture.  */
  set_gdbarch_return_value (gdbarch, zpu_return_value);
  set_gdbarch_breakpoint_kind_from_pc (gdbarch, zpu_breakpoint_kind_from_pc);
  set_gdbarch_sw_breakpoint_from_kind (gdbarch, zpu_sw_breakpoint_from_kind);
  set_gdbarch_print_insn (gdbarch, print_insn_zpu);

  /* Register architecture.  */
  set_gdbarch_pseudo_register_read (gdbarch, zpu_pseudo_register_read);
  set_gdbarch_pseudo_register_write (gdbarch, zpu_pseudo_register_write);
  set_gdbarch_num_regs (gdbarch, ZPU_NUM_REGS);
  set_gdbarch_num_pseudo_regs (gdbarch, ZPU_NUM_REGS);
  set_gdbarch_sp_regnum (gdbarch, ZPU_SP_REGNUM);
  set_gdbarch_pc_regnum (gdbarch, ZPU_PC_REGNUM);
  set_gdbarch_ps_regnum (gdbarch, ZPU_FP_REGNUM);
  set_gdbarch_deprecated_fp_regnum (gdbarch, ZPU_FP_REGNUM);

  /* Functions to supply register information.  */
  set_gdbarch_register_name (gdbarch, zpu_register_name);
  set_gdbarch_register_type (gdbarch, zpu_register_type);
  set_gdbarch_print_registers_info (gdbarch, zpu_print_registers_info);
  set_gdbarch_register_reggroup_p (gdbarch, zpu_register_reggroup_p);

  /* Functions to analyze frames.  */
  set_gdbarch_skip_prologue (gdbarch, zpu_skip_prologue);
  set_gdbarch_inner_than (gdbarch, core_addr_lessthan);
  set_gdbarch_frame_align (gdbarch, zpu_frame_align);
  ...
  set_gdbarch_push_dummy_code (gdbarch, zpu_push_dummy_code);
  set_gdbarch_push_dummy_call (gdbarch, zpu_push_dummy_call);
  set_gdbarch_dummy_id (gdbarch, zpu_dummy_id);

  /* Frame unwinders.  Use DWARF debug info if available, otherwise use our own
     unwinder.  */
  dwarf2_append_unwinders (gdbarch);
  frame_unwind_append_unwinder (gdbarch, &zpu_frame_unwind);
  ...
  if (tdesc_has_registers (info.target_desc))
    {
      ...
      feature = tdesc_find_feature (info.target_desc, "org.gnu.gdb.zpu.cpu");
      if (feature == NULL)
	goto no_tdata;
      ...
      valid_p = 1;
      for (i = ZPU_ZERO_REGNUM; i <= ZPU_LAST_FP_REGNUM; ++i)
        valid_p &= tdesc_numbered_register (feature, tdesc_data, i,
                                            zpu_gdb_reg_names[i]);
      for (i = ZPU_FIRST_CSR_REGNUM; i <= ZPU_LAST_CSR_REGNUM; ++i)
        {
          char buf[20];
          sprintf (buf, "csr%d", i - ZPU_FIRST_CSR_REGNUM);
 ...
 no_tdata:
  ...
  for (i = 0; i < ARRAY_SIZE (zpu_register_aliases); ++i)
    user_reg_add (gdbarch, zpu_register_aliases[i].name,
		  value_of_zpu_user_reg, &zpu_register_aliases[i].regnum);
...
{
  gdbarch_register (bfd_arch_zpu, zpu_gdbarch_init, NULL);

  /* Add root prefix command for all "set zpu"/"show zpu" commands.  */
  add_prefix_cmd ("zpu", no_class, set_zpu_command,
      _("RISC-V specific commands."),
      &setzpucmdlist, "set zpu ", 0, &setlist);

  add_prefix_cmd ("zpu", no_class, show_zpu_command,
      _("RISC-V specific commands."),
      &showzpucmdlist, "show zpu ", 0, &showlist);
